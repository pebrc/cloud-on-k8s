...
func (t *TanzuDriver) create() error {
	...
	if err := t.dockerizedTanzuCmd("management-cluster", "create", "--file", cfgPathInContainer, "--verbose", "9").Run(); err != nil {
		return err
	}
	log.Println("Creating workload cluster")
	if err := t.dockerizedTanzuCmd("cluster", "create", t.plan.ClusterName, "--file", cfgPathInContainer, "--verbose", "9").Run(); err != nil {
		return err
	}
