...
func (v *ResourceValidator[T]) preValidate(ctx context.Context, obj T) (skip bool, err error) {
	...
	if v.namespaceMatcher.SelectorEnabled() {
		...
		// webhooks is actually registered. Unmanaged namespaces are silently let
		// through instead of denied, so an operator never rejects a request for a
		// namespace it doesn't manage.
